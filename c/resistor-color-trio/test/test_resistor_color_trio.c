...
// Added test to cover 3300 case
static void test_orange_orange_red(void)
{
   // TEST_IGNORE();
   resistor_value_t actual =
       color_code((resistor_band_t[]){ ORANGE, ORANGE, RED });
   TEST_ASSERT_EQUAL_UINT16(3300, actual.value);
   TEST_ASSERT_EQUAL(OHMS, actual.unit);
}
...
int main(void)
{
   ...
   RUN_TEST(test_orange_orange_red);
